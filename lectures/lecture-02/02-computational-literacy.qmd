...
## Activity/Discussion: Modern "Computers"
...
- What devices do you use regularly that are essentially computers, even if they don't look like a traditional desktop or laptop?
- [Share some examples!]{.alert} 

:::{.fragment}
This helps us see how broadly the concept of "computing" applies today!
:::
...
Let's look at how these #RRGGBB values translate to actual colours:
...
- **Shades of Grey:**
  - When R, G, and B values are equal, you get a shade of grey.
  - `#808080`: A medium grey (Red=80, Green=80, Blue=80). `80` hex is 128 decimal, the midpoint of 0-255.
  - `#C0C0C0`: Silver (a lighter grey). `C0` hex is 192 decimal.
:::
...
:::{style="margin-top:30px; font-size: 22px;"}
Web developers often use a few more tricks with hex colours:

- **3-Digit Shorthand (`#RGB`):**
  - If each pair of hex digits in a 6-digit code is the same (e.g., RR, GG, BB where R1=R2, G1=G2, B1=B2), you can use a 3-digit shorthand.
...
- **Transparency (Alpha Channel):**
  - Sometimes you need to specify the opacity of a colour. This is done using an "alpha" channel.
  - **8-Digit Hex (`#RRGGBBAA`):** The last two hex digits (`AA`) represent the alpha value.
    - `00` is fully transparent (invisible).
    - `FF` is fully opaque (solid).
    - Values in between give partial transparency (e.g., `80` is 50% opaque).
    - Example: `#FF000080` would be a semi-transparent red.
...
Understanding these formats is crucial for anyone working with web design or digital graphics!
:::
...
# Next lecture {background-color="#2d4563"}
...
- **Representing Text**: How characters are encoded using ASCII and Unicode.
- **The Genesis of Programming**: Early pioneers and the first programming languages.
- **Assembly Language**: Getting closer to the machine.
- **Low-level vs. High-level Languages**: Understanding the spectrum of programming languages.
- **Compiled vs. Interpreted Languages**: Different approaches to executing code.
- **Introduction to the Operating System**: The role of the OS, kernel, and user space.
- **The Shell**: Our gateway to interacting with the OS.
:::
