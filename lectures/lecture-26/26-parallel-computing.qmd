...
subtitle: Lecture 26 - Parallel Computing
author:
  - name: Danilo Freire
...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Parallel Computing](https://raw.githack.com/danilofreire/qtm350-summer/main/lectures/lecture-26/26-parallel-computing.html)"
transition: slide
...
# Hello, everyone! 😊 <br> {background-color="#2d4563"}
...
:::{.columns}
:::{.column width="50%"}
- We are going to use `joblib` and `dask` for parallel computing
- Since `dask` is under active development, it is best to use a virtual environment and install a stable version
- Let's create one using `venv`:
```bash
python3 -m venv .venv-dask
```
...
- On Windows:
```bash
source .venv-dask/Scripts/activate
```
:::

:::{.column width="50%"}
- Once the environment is activated, they should run the following `pip install` commands:
```bash
...
- And we're done! We will talk about virtual environments in more detail in a future lecture 🤓
- If you want to deactive and delete the virtual environment, you can run:
```bash
deactivate
rm -rf .venv-dask
```
...
```
- And then activate/deactivate it with:
```bash
conda activate venv-dask
conda deactivate
```
- To delete the environment, you can run:
```bash
conda remove --name venv-dask --all
```
:::
:::
:::
...
:::{.columns}
:::{.column width="50%"}
- A key tool that we will use later is `map`
...
- Python has provided a `map` function in the standard library:
...
- The built-in `map` function is more flexible than ours, so you should use that one! 😂
::: 
:::
:::
...
- We will use the `%timeit` command to measure the time it takes to run a function
...
%timeit [calculation() for i in range(4)]
``` 
:::
...
%timeit Parallel(n_jobs=4)(delayed(calculation)() for i in range(4))
```
...
- Dask provides a [more efficient way](https://docs.dask.org/en/latest/dataframe-create.html) to read and write `.csv` files
