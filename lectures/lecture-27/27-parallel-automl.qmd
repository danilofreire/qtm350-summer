...
subtitle: Lecture 27 - Parallelising Data Analysis with Dask and AutoML
author:
  - name: Danilo Freire
...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Parallel Computing II](https://raw.githack.com/danilofreire/qtm350-summer/main/lectures/lecture-27/27-parallel-automl.html)"
transition: slide
...
:::{style="margin-top: 30px; font-size: 22px;"}
- Before we start, let's make sure we are using the virtual environment we created in the previous class:

```bash
conda create -n venv-dask -c conda-forge python=3.10 dask-sql=2024.5.0 dask=2024.4.1 dask-ml=2024.4.4 ipykernel=6.29.3 joblib=1.3.2 numpy=1.26.4 pandas=2.2.1 scikit-learn=1.4.2 tpot=0.12.2 prophet -y
...
- If you are using Anaconda, you can activate the environment with:
```{verbatim}
conda activate venv-dask
```

- If you are using `venv`, you can activate the environment with:
```{verbatim}
source venv-dask/bin/activate # macOS/Linux
source venv-dask/Scripts/activate # Windows
```

- The `venv-dask` environment should have all the necessary packages installed!
:::
...
- [Dask ML](https://ml.dask.org/) is built on top of Dask Arrays and DataFrames, allowing you to scale your machine learning workflows to large datasets
...
:::{style="margin-top: 30px; font-size: 22px;"}
- First, let's import the necessary modules, mainly scikit-learn:
...
- The model was trained in 4 seconds, which is pretty fast
- However, the dataset is small, so we are not memory constrained
- The model is not so complex, so we are not compute constrained either
...
- Apart from this, you can always use cloud computing services like AWS, Google Cloud, Azure...
...
- You should have TPOT installed in your `venv-dask` environment, but if you don't, you can install it with:
```{verbatim}
conda install -c conda-forge tpot=0.12.2
```

- I'm using Python 3.10.18 for this example (as suggested in the previous lecture), but your mileage may vary
:::
...
```{verbatim}
Generation 5 - Current best internal CV score: 0.985895635412364
                                                                              
Best pipeline: KNeighborsClassifier(GaussianNB(input_matrix), n_neighbors=2, p=2, weights=distance)
0.9844444444444445
...
- If you installed the `venv-dask` environment, you should already have `Prophet` installed
:::

:::{.column width="50%"}
- Large datasets are not the only type of scaling challenge teams run into
- In this example we will focus once more on [model complexity]{.alert}
