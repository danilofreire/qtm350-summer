...
subtitle: Lecture 19 - Cloud Computing II
author:
  - name: Danilo Freire
...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Cloud Computing II](https://raw.githack.com/danilofreire/qtm350-summer/main/lectures/lecture-19/19-cloud2.html)"
transition: slide
...
- Activity 02: [Data analysis on the cloud]{.alert}
...
- You can copy your saved key file to your WSL home directory using `cp` and `~` as the destination
- In some cases, you may need to use `sudo` to run the commands, including `chmod`, `scp`, and `ssh` (use the password you set when installing WSL)
:::
...
- You can use SSH by uploading your key pair to CloudShell (click on `Actions` and then `Upload file`)
